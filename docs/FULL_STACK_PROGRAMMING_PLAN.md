...
> **This repository's role:** phases 0–1 and 24–36 are primarily owned here: JoshBootloader, JoshFirmware, JoshBIOS, future JoshEC integration and the path from firmware into the canonical Josh kernel. The local `kernel/` remains a regression/test payload, not the canonical OS kernel.
